...
def get_used_procedure_codes(data_path):
    # Read used proc codes from file
    codes = []
    with open(f'{data_path}/used_proc_codes.csv', 'r') as f:
        for code in f:
...
def prepare_procedures_subset(data_path):
    # Read the procedure codes used in MIMIC-IV
    used_codes = get_used_procedure_codes(data_path)
    ...
    df.to_csv(f'{data_path}/hrchy_subset_proc.csv', sep=';', index=False)
