...
class MimicIVDataExtractor(DataExtractor):
    ...
    def person(self, sub):
        return f"""select distinct P.person_id as {sub}
                   from `{self.query_prefix}.cdm_condition_occurrence` CO
                   join `{self.query_prefix}.cdm_person` P on P.person_id = CO.person_id
                   join `{self.query_prefix}.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                   join `{self.query_prefix}.ccs_single` CCS on CCS.icd9 = CO.condition_source_value
                   join `{self.query_prefix}.icd9_filtered` FILT on FILT.level4 = CO.condition_source_value
                   where C.vocabulary_id = 'ICD9CM'
                   and CO.condition_source_value in (
                       select distinct FILT.level4 from `{self.query_prefix}.icd9_filtered` as FILT
                       join `{self.query_prefix}.cdm_condition_occurrence` CO on CO.condition_source_value = FILT.level4
                       join `{self.query_prefix}.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                       where C.vocabulary_id = 'ICD9CM'
    ...
    def diagnosis(self, sub):
        return f"""select distinct FILT.level4 as {sub} 
                   from `{self.query_prefix}.icd9_filtered` as FILT
                   join `{self.query_prefix}.cdm_condition_occurrence` CO on CO.condition_source_value = FILT.level4
                   join `{self.query_prefix}.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                   where C.vocabulary_id = 'ICD9CM'
                   group by FILT.level4
    ...
    def medication(self, sub):
        return f"""select distinct C2.concept_code as {sub}
                   from `{self.query_prefix}.cdm_drug_exposure` DE
                   join `{self.query_prefix}.cdm_person` P on P.person_id = DE.person_id
                   join `{self.query_prefix}.voc_concept` C1 on C1.concept_id = DE.drug_concept_id
                   join `{self.query_prefix}.voc_concept_ancestor` CA on CA.descendant_concept_id = C1.concept_id
                   join `{self.query_prefix}.voc_concept` C2 on C2.concept_id = CA.ancestor_concept_id
    ...
    def labtest(self, sub):
        return f"""select distinct C.concept_code || '-N' as {sub}
                   from `{self.query_prefix}.cdm_measurement` M
                   join `{self.query_prefix}.voc_concept` C on M.measurement_concept_id = C.concept_id
    ...
    def procedure(self, sub):
        return f"""SELECT DISTINCT C.concept_code as {sub}
                   FROM `{self.query_prefix}.cdm_procedure_occurrence` CO
                   join `{self.query_prefix}.voc_concept` C on C.concept_id = CO.procedure_concept_id
    ...
    def person_labtest(self, sub, obj):
        return f"""select distinct M.person_id as {sub}, CASE when L.flag is NULL then C.concept_code || '-N' else C.concept_code || '-A' end as {obj} 
                   from `{self.query_prefix}.cdm_measurement` M
                   join `{self.query_prefix}.voc_concept` C on M.measurement_concept_id = C.concept_id
                   join `{self.query_prefix}.src_labevents` L on M.trace_id = L.trace_id"""
    ...
    def person_features(self):
        return f"""select CP.person_id as person, CP.gender_concept_id as gender, CP.race_concept_id as race, CP.ethnicity_concept_id as ethnicity, SP.anchor_age as age 
                   from `{self.query_prefix}.cdm_person` CP
                   join `{self.query_prefix}.src_patients` SP on CAST(SP.subject_id as string) = CP.person_source_value"""
    ...
    def atc_code_features(self):
        return f"""select distinct C2.concept_code as medication, C3.concept_code as atc_code
                   ...
                   join `{self.query_prefix}.voc_concept_ancestor` CA on CA.descendant_concept_id = C1.concept_id
                   join `{self.query_prefix}.voc_concept` C2 on C2.concept_id = CA.ancestor_concept_id
                   join `{self.query_prefix}.voc_concept_relationship` CR on C2.concept_id = CR.concept_id_1
                   join `{self.query_prefix}.voc_concept` C3 on C3.concept_id = CR.concept_id_2
                   where DE.drug_concept_id > 0
                   and CA.min_levels_of_separation = 1
                   and C2.vocabulary_id = 'RxNorm'
                   and C2.concept_class_id = 'Clinical Drug Form'
