...
def get_used_loinc_lab_codes(data_path):
    # Read used loinc codes from file

    codes = set()
    with open(f'{data_path}/used_loinc_codes.csv', 'r') as f:
        for code in f:
...
def prepare_loinc_hierarchy_subset(data_path):
    # Read the full loinc axial hierarch
    full_hierarchy = pd.read_excel(f'{data_path}/full_hrchy_loinc.xlsx')

    # Read the loinc codes used in MIMIC-IV
    used_codes = get_used_loinc_lab_codes(data_path)
    ...
    subset_hrchy.to_csv(f'{data_path}/hrchy_subset_loinc.csv', sep=';', index=False)
