...
from datasetmodule.utils.utils import set_random_seed
...
def extract_graph_edges():
    ...
    file_path = os.path.join(config.path['output_fold'], 'sparse.csv')
...
def convert_sparse_to_parquet():
    file_path = os.path.join(os.getcwd(), '../data', 'sparse.micro')
    df = pd.read_csv(file_path)

    out_path = os.path.join(os.getcwd(), '../output', 'graphlet_features.parquet')
    df.to_parquet(out_path)

convert_sparse_to_parquet()
