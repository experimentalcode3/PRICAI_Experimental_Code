...
class Graph:
    ...
    def add_aux_feature(self, entity: str, feature: dict):
        ...
        if len(feature) != len(entity_ids):
            ...
            # If we are missing features for some entities, we random initialize them
            if len(feature) < len(entity_ids):
                random_string = f'with {len(entity_ids) - len(feature)} random initialized'
                feature = self.random_initialize_missing_features(feature, entity_ids)
        ...
        print(f'Added {feature[0].shape[0]} dim {entity} feats to graph {random_string}')

    def random_initialize_missing_features(self, feature: dict, entity_ids: set) -> dict:
        # Create random tensor of same length as the other embeddings
        random_tensor = th.rand(list(feature.items())[0][1].shape, dtype=th.float32)

        # For entities with no embedding, we create a random initialized one
        for entity_id in entity_ids:
            if entity_id not in feature:
                feature[entity_id] = random_tensor
